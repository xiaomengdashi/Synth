...
async def process_task(task_id: str, url: str, config: ModelConfig):
    ...
    try:
        ...
        if is_twitter:
            TASKS_DB[task_id]["current_step"] = "正在抓取推文内容..."
            ...
            try:
                # 为了解决各种 Python SSL 的疑难杂症，对于推特抓取，直接抛弃 Python 的所有内置网络库，使用系统底层的 curl 进行请求
                # 这能够直接绕过 `[SSL: SSLV3_ALERT_HANDSHAKE_FAILURE]` 等所有由于底层库 OpenSSL 兼容性引起的问题
                result = subprocess.run(['curl', '-s', '-k', '-L', api_url], capture_output=True, text=True, timeout=30)
                if result.returncode != 0:
                    raise Exception(f"主节点 curl 失败: {result.stderr}")
                ...
                try:
                    # 备用方案：尝试使用 fxtwitter API
                    backup_url_curl = api_url.replace("api.vxtwitter.com", "api.fxtwitter.com")
                    result_backup = subprocess.run(['curl', '-s', '-k', '-L', backup_url_curl], capture_output=True, text=True, timeout=30)
                    if result_backup.returncode != 0:
                        raise Exception(f"备用节点 fxtwitter curl 失败: {result_backup.stderr}")
                    
                    tweet_data = json.loads(result_backup.stdout)
                    if 'error' in tweet_data or ('code' in tweet_data and tweet_data['code'] != 200):
                         raise Exception(f"API 返回错误: {tweet_data}")
                except Exception as backup_e:
                    # 终极方案：尝试使用 twittpr API (注意它也是一个包装的 API)
                    try:
                        backup_url_2_curl = api_url.replace("api.vxtwitter.com", "api.twittpr.com")
                        result_final = subprocess.run(['curl', '-s', '-k', '-L', backup_url_2_curl], capture_output=True, text=True, timeout=30)
                        if result_final.returncode != 0:
                            raise Exception(f"终极节点 twittpr curl 失败: {result_final.stderr}")
                        
                        tweet_data = json.loads(result_final.stdout)
                        if 'error' in tweet_data or ('code' in tweet_data and tweet_data['code'] != 200):
                             raise Exception(f"API 返回错误: {tweet_data}")
                    except Exception as final_e:
                        raise Exception(f"无法获取推文信息 (所有节点及抓取方式均失败): 主节点:{req_e} | 备用节点:{backup_e} | 终极节点:{final_e}")
...
要求：
1. 提取出视频的核心观点，分点列出。
2. 语言需要专业、简练，适合科技和 AI 领域的读者。
3. 必须使用 Markdown 格式（如标题、引用、列表等）。
4. 在文章开头需要有一段简短的摘要。
            """
        ...
        source_type = 'bilibili' if 'bilibili' in url else 'douyin' if 'douyin' in url else 'wechat' if 'weixin' in url else 'x' if ('x.com' in url or 'twitter.com' in url) else 'other'
        ...
        TASKS_DB[task_id]["current_step"] = f"处理失败: {str(e)}"
        ...
        try:
            ...
            async with AsyncSessionLocal() as session:
                ...
                if db_task:
                    ...
                    db_task.current_step = f"处理失败: {str(e)}"
